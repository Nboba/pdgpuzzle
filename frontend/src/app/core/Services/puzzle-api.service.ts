...
import { RequestPuzzleModel,RequestPetitionPuzzleModel,ResponsePuzzleModel } from '../Models/request-puzzle-model';
...
export class PuzzleApiService {
  ...
  public updateCheck(checkPuzzle: number): void {
    this._indexPuzzles()[checkPuzzle].selected = !this._indexPuzzles()[checkPuzzle].selected;
  }
  ...
  public getSelectedPuzzles(): ResponsePuzzleModel[] {
    return this._indexPuzzles().filter((check) => check.selected === true).map((checks) => this._puzzleResult()[checks.index]);
  }

  public getMetaData(index:number):[number,number]{
    return [this._puzzleResult()[index].NMoves,this._puzzleResult()[index].NSolutions];
  }
  ...
  getNPuzzle(request: RequestPetitionPuzzleModel):Observable<RequestPuzzleModel>{
    console.log(`${this.API_URL}/GetNPuzzles`);
    return this.http.post<RequestPuzzleModel>(`${this.API_URL}/PuzzleApi/GetNPuzzles`, request);
  }
