...
from .mechanics import MOVES, step, win, EMPTY, WALL, DOOR, ENEMY, BLOCK, PLAYER
from .solver import nSolutions
...
def repairDoors(child1,child2):
    doors1 = np.argwhere(child1 == DOOR)
    doors2 = np.argwhere(child2 == DOOR)
    if len(doors1) == 2:
        door2 = random.choice(doors1)
        child1[door2[0],door2[1]] = WALL
        child2[door2[0],door2[1]] = DOOR
    elif len(doors2) == 2:
        door2 = random.choice(doors2)
        child2[door2[0],door2[1]] = WALL
        child1[door2[0],door2[1]] = DOOR
    return child1,child2

def repairPlayer(child1,child2):
    players1 = np.argwhere(child1 == PLAYER)
    players2 = np.argwhere(child2 == PLAYER)
    if len(players1) == 2:
        player2 = random.choice(players1)
        child1[player2[0],player2[1]] = EMPTY
        child2[player2[0],player2[1]] = PLAYER
    elif len(players2) == 2:
        player2 = random.choice(players2)
        child2[player2[0],player2[1]] = EMPTY
        child1[player2[0],player2[1]] = PLAYER
    return child1,child2
...
def mutate(dungeon):
    tiles = np.argwhere((dungeon != WALL) & (dungeon != EMPTY))
    ...
    if mutateTile == DOOR:
        ...
        dungeon[tile[0],tile[1]] = WALL
        dungeon[newDoor[0],newDoor[1]] = DOOR
    elif np.random.rand() <= 0.5 and mutateTile in [ENEMY,BLOCK]:
        if mutateTile == ENEMY:
            dungeon[tile[0],tile[1]] = BLOCK
        else:
            dungeon[tile[0],tile[1]] = ENEMY
    else:
        emptys = np.argwhere(dungeon == EMPTY)
        newEntity = random.choice(emptys)
        dungeon[tile[0],tile[1]] = EMPTY
        if mutateTile == PLAYER:
            dungeon[newEntity[0],newEntity[1]] = PLAYER
        else:
            dungeon[newEntity[0],newEntity[1]] = random.choice([ENEMY,BLOCK])
    return dungeon
...
def initialPopulation(population):
    ...
    for dungeon in population:
        nSol,minMoves = nSolutions(dungeon)
        if isFeasible(nSol):
...
def FI2Pop(population,maxIter, nPop,mutationFactor,maxMoves): 
    ...
    return np.array(feapop[0].solution)